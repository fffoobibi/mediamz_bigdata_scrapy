...
from scrapy_selenium.middlewares import make_response
from scrapy_selenium.pipelines import SaveToSqlPipesline
from scrapy_selenium import RedisIpPool, g, dict_to_obj, spider, driver, request
from selenium.webdriver.common.by import By
from selenium.webdriver.support import expected_conditions as EC
from mediamz_crawl.spiders.ins_scraper import InstagramScraper
...
class IpPool(RedisIpPool):
    ...
    def _request(self, url, **kwargs):
        ...
        elif url is not None and 'instagram' in url.lower():
...
class InsTools:
    ...
    def request_log(item: dict, dm: 'SaveToSqlPipesline', account: dict, url: str, logger, user_data=None):
        ...
        try:
            user_name = re.findall(
                r'instagram\..*?/(.*?)', url)[0].split('/')[0]
        except:
            user_name = url.strip(
                'https://www.instagram.com/').strip('/').strip()

        if user_data:
            user = user_data
        else:
            proxies = {'http': f'http://{account["ip"]}'}
            logger.info(f'--- ins spider use proxy {proxies}---')
            scraper = InstagramScraper(
                login_user=account['user'], login_pass=account['pwd'], proxies=proxies)
            user = scraper.get_shared_data_userinfo(user_name)
            if user is None:
                logger.info(f'---ins spider use none user: {user_name}---')
                scraper = InstagramScraper()
                user = scraper.get_shared_data_userinfo(user_name)
            if user is None:
                logger.info(
                    f'---ins spider use cookies user: {user_name} proxy: {proxies} account:{account}---')
                scraper = InstagramScraper(
                    login_user=account['user'], login_pass=account['pwd'], proxies=proxies)
                scraper.load_login_cookies(Tools.redis_client, 3)
                user = scraper.get_shared_data_userinfo(user_name)

        _data = dict(create_time=time.time(), celebrity_id=row.id)

        if user is not None:
            logger.info('-----ins celebrity success----')
            ...
            logger.info('---ins celebrity fail---')
            return False
    ...
    def _save_redis_cookies(account, cookies, redis_client: Redis):
        ...
        redis_client.setex(InsTools.prefix +
                           account['user'], 60 * 60 * 3, content)
    ...
    def _instagram_login(account: dict, redis_client: Redis = None) -> bool:
        ...
        try:
            ...
            if redis_client is not None:
                InsTools._save_redis_cookies(account,
...
class Crawl:
    ...
    def crawl_promoter_detail(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _tiktok_promoter_detail(item, dm, **kw):
            ...
            try:
                ...
                _data = dict(crawl_platform='tiktok',
                             is_crawl=1, crawl_fans=fans)
                if len(store) > 0:
        ...
        def _ytb_promoter_detail(item, dm, **kw):
            ...
            try:
                ...
                try:
                    ...
                    if '小时' in dt:
                        dt = re.findall('(\d+)小时', dt)[0]
                        dt = datetime.now() - timedelta(hours=int(dt))
                    else:
                        dt = str(dt).replace(
        ...
        funcs = [_ins_promoter_detail,
                 _tiktok_promoter_detail, _ytb_promoter_detail]

        try:
            funcs[type](item, dm, **kw)
    ...
    def crawl_analysisn(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _ins_crawl_analysisn(item: dict, dm: SaveToSqlPipesline, **kw):
            ...
            account = kw.pop('account')
            try:
                if not InsTools.get_with_cookies(account, row.channel_url):
                    logger.error(f'ins 登录失败, account: {account}')
                    raise Exception(f'--- ins登录失败 --- account: {account}')
                ...
                try:
                    for lst in post_list:
                        ...
                        for ai in a:
                            ...
                            logger.info(
                                f"get url for item is: {tmp_parsed.path}")
                            if ad_parsed.path == tmp_parsed.path:
                                ...
                                logger.info('-' * 30)
                                logger.info(
                                    f"comments count is: {comments_count}")
                                logger.info('-' * 30)
                                break
                ...
                if store:
                    ...
                    _list_like = list(
                        map(InsTools.parse_ins_number, _list_like))
                    if len(_list_like) > 0:
                ...
                if should_crawl(row.last_crawler_time, row.crawler_mode):
            ...
            try:
                ...
                for i in items:
                    ...
                    if tmp.path == parsed.path:
                        logger.info('-' * 30 + 'matched url')
                        try:
                            advert_watch_count = i.find_element_by_tag_name(
                                'strong').text
                        except:
                            pass
                        break
                ...
                logger.info('-' * 40)
                logger.info("advert watch count is: {}".format(
                    advert_watch_count))
                logger.info('-' * 40)
                avg_watch = 0

                parse_tiktok_number = TiktokTools.parse_tiktok_number
                ...
                if len(_a_list) > 0:
                    ...
                    for i in _a_list:
                        ...
                        try:
                            _like = driver.find_element_by_xpath(
                                like_xpath).text
                ...
                if should_crawl(row.last_crawler_time, row.crawler_mode):
                    ...
                    try:
                        _watch = advert_watch_count != 0 and parse_tiktok_number(
                            str(advert_watch_count)) or 0
                        logger.info(_watch)
                    except Exception as e:
                        logger.error(e, exc_info=True)
                        _watch = 0
                    ...
                    _data.update(last_crawler_time=last_time, adverts_plays=_watch, adverts_likes=_like,
                ...
                if len(_data.keys()) > 0:
                    dm.update(Tbl.tbl_marketing_analysisn,
                              ('id', row.id), _data, logger)
        ...
        funcs = [_ins_crawl_analysisn,
                 _tiktok_crawl_analysisn, _ytb_crawl_analysisn]
    ...
    def crawl_mcn(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _tiktok_crawl_mcn(item: dict, dm: SaveToSqlPipesline, **kw):
            ...
            try:
                logger.info(
                    f'---------resource url {row.resource_url}-----------')
    ...
    def crawl_total_resource(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _tiktok_crawl_total_resource(item: dict, dm: SaveToSqlPipesline, **kw):
            ...
            try:
                ...
                if len(_a_list) > 0:
                    ...
                    if len(_like_list) > 0:
                        _like_list = list(
                            map(TiktokTools.parse_tiktok_number, _like_list))
                        _avg = reduce(lambda x, y: x + y,
                                      _like_list) / len(_like_list)
                        logger.info(f'tiktok avg is : {_avg}')
                        avg_like = _avg
        ...
        funcs = [_ins_crawl_total_resource,
                 _tiktok_crawl_total_resource, _ytb_crawl_total_resource]
    ...
    def crawl_partner(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _ins_crawl_partner(item: dict, dm: SaveToSqlPipesline, **kw):
            ...
            try:
                ...
                if re.search(r'/accounts/login', cur_url):
                    # logger.info('帐号可能被封？')
                    raise Exception(f'--- ins 登录失败 --- account: {account}')
        ...
        def _ytb_crawl_partner(item: dict, dm: SaveToSqlPipesline, **kw):
            ...
            try:
                ...
                _data = dict(subscribes=subscribe,
                             last_crawler_time=time.time())
                logger.info('-' * 40)
    ...
    def crawl_log(type: int, item: dict, dm: SaveToSqlPipesline, **kw):
        ...
        def _ins_crawl_log(item: dict, dm: SaveToSqlPipesline, **kw):
            row = dict_to_obj(item)
            logger = spider.logger
            account = kw.pop('account')
            ...
            try:
                ...
                # if InsTools.request_log(item, dm, account, request.url, logger):
                #     logger.info('--- ins crawl spider ok -----')
                #     return
                if not InsTools.get_with_cookies(account, channel_url):
                    logger.error(f' --- ins 登录失败 --- account: {account}')
                    raise Exception(f'ins登录失败, account: {account}')
                ...
                for lst in post_list:
                    ...
                    if len(store) == 6:
                        break
                if post_list:
                    date_link = post_list[0].find_elements_by_tag_name(
                ...
                if fans:
                    ...
                    _data.udpate(now_followers=row.now_followers)
                ...
                if store:
                    ...
                    _list_like = list(
                        map(InsTools.parse_ins_number, _list_like))
                    if len(_list_like) > 0:
                        total = reduce(lambda x, y: x + y, _list_like)
                        avg = total / \
                            len(_list_like)
                        _data.update(now_avg_watchs=avg, watchs=total)

                else:
                    _data.update(now_avg_watchs=row.now_avg_watchs,
                                 watchs=row.watchs or 0)
                ...
                try:
                    ...
                    logger.info('get last_update_time error')
                    logger.info(e, exc_info=True)

                _data.update(last_crawler_time=time.time())
                ...
                if _data.get('now_avg_watchs', 0) > row.now_avg_watchs and _data.get('now_followers',
                                                                                     0) > row.now_followers:
                    channel_status = 2
                elif _data.get('now_avg_watchs', 0) < row.now_avg_watchs and _data.get('now_followers',
                                                                                       0) < row.now_followers:
                    channel_status = 4
                else:
                    channel_status = row.channel_status or 1
                ...
                logger.error('--ins error--', exc_info=True)
                raise

        def _tiktok_crawl_log(item: dict, dm: SaveToSqlPipesline, **kw):
            logger = spider.logger
            row = dict_to_obj(item)
            channel_url = row.channel_url
            db = dm
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        if len(str(dt)) < 10:
                            ...
                            logger.info(
                                f'----- tiktok click get time video-url: {video_url} -----')
                            driver.get(video_url)
                            us = driver.u(
                                'h4[class*="author-nickname"]', 5).text
                            if re.findall(r'(\d+)(?=小時|h)', us):
                                hs = re.findall(r'(\d+)(?=小時|h)', us)[0]
                                dt = (datetime.now() -
                                      timedelta(hours=int(hs))).timestamp()
                            elif re.findall(r'(\d+)(?=天|d)', us):
                                hs = re.findall(r'(\d+)(?=天|d)', us)[0]
                                dt = (datetime.now() -
                                      timedelta(days=int(hs))).timestamp()
                        logger.info(f'------parsed url dt is: {dt}-----------')
                        if dt:
                            _data.update(last_update_time=dt)
                    except:
                        logger.error('parse tiktok time fail', exc_info=True)
                        try:
                            driver.get_screenshot_as_file(
                                'tiktok_time_fail.png')
                        except:
                            ...
                ...
                _celebrity_data.pop('celebrity_id')
                _celebrity_data.pop('watchs', None)
                _celebrity_data.update(
                    channel_status=channel_status, id=row.id)
        ...
        def _ytb_crawl_log(item: dict, dm: SaveToSqlPipesline, **kw):
            row = dict_to_obj(item)
            logger = spider.logger
            logger.info('----- crawl spider log ------')
            t = 0 if row.last_crawler_time is None else row.last_crawler_time
            if (time.time() - t) >= Tbl.spider_limit:
                ...
                try:
                    ...
                    subscriber = driver.u('#subscriber-count', 5)
                    subscribe_count = YtbTools.parse_ytb_number(
                        subscriber.text)  # 订阅数量
                    items = driver.find_elements_by_css_selector(
                        '#items > ytd-grid-video-renderer')  # 首页最近6个视频
